...
def get_file_timerange(file_path):
    """Lấy mốc thời gian bắt đầu và kết thúc của một file log (Hỗ trợ file cực lớn và chưa sort)"""
    ...
    try:
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            ...
            # Lấy dòng cuối (Dùng seek để tối ưu cho file GB)
            f.seek(0, os.SEEK_END)
            filesize = f.tell()
            if filesize > 0:
                # Đọc ngược từ cuối file khoảng 4096 bytes để tìm dòng cuối cùng hoàn chỉnh
                offset = min(filesize, 4096)
                f.seek(filesize - offset)
                last_chunk = f.read(offset)
                lines = last_chunk.splitlines()
                if lines:
                    # Duyệt ngược từ cuối chunk để tìm bản ghi JSON hợp lệ đầu tiên
    ...
    if not first_ts or not last_ts: return first_ts, last_ts
    return min(first_ts, last_ts), max(first_ts, last_ts)

def load_config():
    config_path = "config.json"
    defaults = {
        "alerts": "./data/alerts_trigger_botsv1.json",
        "sysmon": "./data/sysmon_logs_botsv1.json",
        "output": "./reports/triage_context.json"
    }
    if os.path.exists(config_path):
        try:
            with open(config_path, 'r') as f:
                cfg = json.load(f)
                paths = cfg.get("default_paths", {})
                return {
                    "alerts": paths.get("alerts", defaults["alerts"]),
                    "sysmon": paths.get("sysmon", defaults["sysmon"]),
                    "output": os.path.join(paths.get("reports_dir", "./reports"), "triage_context.json")
                }
        except Exception:
            pass
    return defaults
...
def main():
    ...
    parser.add_argument("--alert-file", default=cfg["alerts"], help="Path to alerts file")
    parser.add_argument("--sysmon-file", default=cfg["sysmon"], help="Path to sysmon file")
    parser.add_argument("--output-file", default=cfg["output"], help="Path to save triage context JSON")
    ...
    if timestamps:
        ...
        sysmon_min, sysmon_max = get_file_timerange(args.sysmon_file)
        if sysmon_min and sysmon_max:
            # Kiểm tra xem mốc thời gian Alert có nằm trong khung thời gian của File Log không
            # Cho phép sai số 1 ngày ở hai đầu
            if not (sysmon_min.date() <= first_seen_obj.date() <= sysmon_max.date()):
                warnings.append(f"CRITICAL TIME MISMATCH: Alerts are from {first_seen_obj.date()}, but Sysmon logs cover {sysmon_min.date()} to {sysmon_max.date()}. Log collection will likely return 0 results.")
