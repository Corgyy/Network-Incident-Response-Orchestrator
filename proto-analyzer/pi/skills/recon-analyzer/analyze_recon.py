...
class ReconAgent:
    def __init__(self, vt_key=None, abuse_key=None):
        ...
        self.abuse_api_key = abuse_key
        self.headers = {"Accept": "application/json"}

    async def check_abuse_ipdb(self, client: httpx.AsyncClient, ip: str) -> dict:
        if not self.abuse_api_key or self.abuse_api_key == "YOUR_ABUSEIPDB_KEY":
    ...
    async def run(self, src_ip: str, output_path: str):
        ...
        if src_ip.startswith("192.168.") or src_ip.startswith("10."):
            query_ip = "23.22.63.114" 

        try:
            async with httpx.AsyncClient(verify=False) as client: # verify=False để tránh lỗi SSL cục bộ nếu có
                task_abuse = self.check_abuse_ipdb(client, query_ip)
                task_vt = self.check_virus_total(client, query_ip)
            ...
            print(f"[-] Lỗi nghiêm trọng trong hàm run: {e}", file=sys.stderr)
